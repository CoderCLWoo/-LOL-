...
@interface HeroViewController ()<RequestDelegate, UITableViewDataSource, UITableViewDelegate>
...
- (void)viewDidLoad {
    ...
    [request getDataFromUrlString:HEROURL];
...
- (void)passValue:(id)responseObject {
    ...
    for (NSDictionary *dic in array) {
        HeroModel *model = [[HeroModel alloc] initWithDictionary:dic];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    HeroModel *model = self.modelArray[indexPath.row];
    
    [cell.imgView sd_setImageWithURL:[NSURL URLWithString:model.img]];
    cell.titleLabel.text = model.title;
    cell.catsLabel.text = model.heroCats;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    HeroModel *model = self.modelArray[indexPath.row];
    currentHeroTableViewController.urlString = [NSString stringWithFormat:@"http://lol.video.luckyamy.com/api/?cat=hero&id=%@&page=1&ap=lol&ver=1.3",model.ids];
